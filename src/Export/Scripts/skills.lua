...
local function sanitiseExportText(text)
	return text and text:gsub("\r", "") or nil
end
...
directiveTable.skill = function(state, args, out)
	...
	local skillBaseTypeName
	if skillGem and not state.noGem then
		gems[gemEffect.Id] = true
		if granted.IsSupport then
			skillName = fullNameGems[skillGem.BaseItemType.Id] and skillGem.BaseItemType.Name or skillGem.BaseItemType.Name:gsub(" Support","")
			out:write('\tname = "', skillName, '",\n')
			if #gemEffect.Description > 0 then
				skillDescription = sanitiseExportText(gemEffect.Description)
				out:write('\tdescription = "', gemEffect.Description:gsub('"','\\"'):gsub('\r',''):gsub('\n','\\n'), '",\n')
			end
		else
			skillName = secondaryEffect and granted.ActiveSkill.DisplayName or trueGemNames[gemEffect.Id] or granted.ActiveSkill.DisplayName
			out:write('\tname = "', skillName, '",\n')
			-- Hybrid gems (e.g. Vaal gems) use the display name of the active skill e.g. Vaal Summon Skeletons of Sorcery
			skillBaseTypeName = granted.ActiveSkill.DisplayName
			out:write('\tbaseTypeName = "', skillBaseTypeName, '",\n')
		end
	else
	...
	if granted.IsSupport then
		...
		if #granted.ActiveSkill.Description > 0 then
			skillDescription = sanitiseExportText(granted.ActiveSkill.Description)
	...
	if exportLocale then
		localeSkills[grantedId] = {
			name = skillName,
			baseTypeName = skillBaseTypeName,
...
directiveTable.__skipOutput = localeOnlyExport
...
local out
if not localeOnlyExport then
	out = io.open("../Data/Gems.lua", "w")
...
for skillGem in dat("SkillGems"):Rows() do
	for _, gemEffect in ipairs(skillGem.GemVariants) do
		if gems[gemEffect.Id] then
			...
			local gemName = fullNameGems[skillGem.BaseItemType.Id] and skillGem.BaseItemType.Name or trueGemNames[gemEffect.Id] or skillGem.BaseItemType.Name:gsub(" Support","")
			local gemBaseTypeName
			local secondaryEffectName
			local tagNames = { }
			if not localeOnlyExport then
			...
			if not skillGem.IsSupport then
				gemBaseTypeName = gemEffect.GrantedEffect.ActiveSkill.DisplayName
				if not localeOnlyExport then
					out:write('\t\tbaseTypeName = "', gemBaseTypeName, '",\n')
				end
			end
			if not localeOnlyExport then
			...
			if #gemEffect.SecondarySupportName > 0 then
				secondaryEffectName = gemEffect.SecondarySupportName
				if not localeOnlyExport then
					out:write('\t\tsecondaryEffectName = "', secondaryEffectName, '",\n')
				end
			end
			if not localeOnlyExport and skillGem.IsVaalGem then
			...
			if exportLocale then
				localeGems[gemEffect.Id] = {
					name = gemName,
					baseTypeName = gemBaseTypeName,
					secondaryEffectName = secondaryEffectName,
					tagString = table.concat(tagNames, ", "),
				}
			end
		end
	end
...
if exportLocale then
	...
	local localeSkillOut = io.open("../Data/Locales/"..exportLocale.."/Skills.lua", "w")
	localeSkillOut:write('-- This file is automatically generated, do not edit!\n')
	localeSkillOut:write('-- Skill localisation data (c) Grinding Gear Games\n\nreturn ')
	writeLuaTable(localeSkillOut, localeSkills, 1)
	localeSkillOut:close()

	local localeGemOut = io.open("../Data/Locales/"..exportLocale.."/Gems.lua", "w")
	localeGemOut:write('-- This file is automatically generated, do not edit!\n')
	localeGemOut:write('-- Gem localisation data (c) Grinding Gear Games\n\nreturn ')
	writeLuaTable(localeGemOut, localeGems, 1)
	localeGemOut:close()
end
