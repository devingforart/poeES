...
local localizedBaseOverlay = { }
local localeBaseOrder = { }
local currentLocaleType
local currentLocaleIndex = 1

local function loadBaseOrder(name)
	local order = { }
	local handle = io.open("../Data/Bases/"..name..".lua", "r")
	if not handle then
		return order
	end
	for line in handle:lines() do
		local key = line:match('^itemBases%["(.-)"%] = {')
		if key then
...
directiveTable.base = function(state, args, out)
	...
	if baseItemType.FlavourTextKey and baseItemType.FlavourTextKey.Text then
		cleanedLines = cleanAndSplit(baseItemType.FlavourTextKey.Text)
	end
	if exportLocale and localeOnlyExport then
		local englishKey = localeBaseOrder[currentLocaleType] and localeBaseOrder[currentLocaleType][currentLocaleIndex]
		currentLocaleIndex = currentLocaleIndex + 1
		if englishKey then
			localizedBaseOverlay[englishKey] = {
				name = displayName,
			}
			if cleanedLines and #cleanedLines > 0 then
				localizedBaseOverlay[englishKey].flavourText = cleanedLines
			end
		end
	end
...
directiveTable.__skipOutput = exportLocale and localeOnlyExport
for _, name in pairs(itemTypes) do
	currentLocaleType = name
	currentLocaleIndex = 1
	if exportLocale and localeOnlyExport then
		localeBaseOrder[name] = loadBaseOrder(name)
	end
...
if exportLocale and localeOnlyExport then
	...
	local out = io.open("../Data/Locales/"..exportLocale.."/ItemBases.lua", "w")
	out:write('-- This file is automatically generated, do not edit!\n')
	out:write('-- Official item base localisation data (c) Grinding Gear Games\n\nreturn ')
	writeLuaTable(out, localizedBaseOverlay, 1)
	out:close()
	print("Item base locale overlay exported.")
	return
end
