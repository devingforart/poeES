...
local ImportTabClass = newClass("ImportTab", "ControlHost", "Control", function(self, build)
	...
	self.controls.sectionCharImport = new("SectionControl", {"TOPLEFT",self,"TOPLEFT"}, {10, 18, 650, 250}, function()
		return tr("import.section.character", "Character Import")
	end)
	self.controls.charImportStatusLabel = new("LabelControl", {"TOPLEFT",self.controls.sectionCharImport,"TOPLEFT"}, {6, 14, 200, 16}, function()
		return "^7" .. tr("import.status_prefix", "Character import status: ") .. self.charImportStatus
	end)

	-- Stage: input account name
	self.controls.accountNameHeader = new("LabelControl", {"TOPLEFT",self.controls.sectionCharImport,"TOPLEFT"}, {6, 40, 200, 16}, function()
		return "^7" .. tr("import.account_name_header", "To start importing a character, enter the character's account name:")
	end)
	...
	self.controls.accountNameGo = new("ButtonControl", {"LEFT",self.controls.accountName,"RIGHT"}, {8, 0, 60, 20}, function()
		return tr("import.start", "Start")
	end, function()
	...
	self.controls.accountNameGo.tooltipFunc = function(tooltip)
		...
		if not self.controls.accountName.buf:match("[#%-]%d%d%d%d$") and self.controls.accountName.buf ~= "" then
			tooltip:AddLine(16, "^7" .. tr("import.missing_discriminator", "Missing discriminator e.g. ") .. self.controls.accountName.buf .. "#1234")
		end
	end
	...
	self.controls.removeAccount.tooltipFunc = function(tooltip)
		tooltip:Clear()
		tooltip:AddLine(16, "^7" .. tr("import.remove_account_tooltip", "Removes account from the dropdown list"))
	end

	self.controls.accountNameMissingDiscriminator = new("LabelControl", {"TOPLEFT",self.controls.accountName,"BOTTOMLEFT"}, {0, 8, 0, 16}, function()
		return "^1" .. tr("import.missing_discriminator_short", "Missing discriminator e.g. #1234")
	end)
	...
	self.controls.accountNameUnicode = new("LabelControl", {"TOPLEFT",self.controls.accountRealm,"BOTTOMLEFT"}, {0, 34, 0, 14}, function()
		return "^7" .. tr("import.account_unicode_note", "Note: if the account name contains non-ASCII characters it must be pasted into the textbox,\nnot typed manually.")
	end)
	...
	self.controls.sessionHeader.label = function()
		return "^7" .. tr("import.session_header", "The list of characters on '%s' couldn't be retrieved. This may be because:\n1. You are missing the discriminator at the end of the account name e.g. #1234\n2. You entered a character name instead of an account name or\n3. This account's characters tab is hidden (this is the default setting).\nIf this is your account, you can either:\n1. Uncheck \"Hide Characters\" in your privacy settings or\n2. Enter a POESESSID below.\nYou can get this from your web browser's cookies while logged into the Path of Exile website.", self.controls.accountName.buf)
	end
	self.controls.sessionHeader.shown = function()
		return self.charImportMode == "GETSESSIONID"
	end
	self.controls.sessionRetry = new("ButtonControl", {"TOPLEFT",self.controls.sessionHeader,"TOPLEFT"}, {0, 122, 60, 20}, function()
		return tr("import.retry", "Retry")
	end, function()
		self:DownloadCharacterList()
	end)
	self.controls.sessionCancel = new("ButtonControl", {"LEFT",self.controls.sessionRetry,"RIGHT"}, {8, 0, 60, 20}, function()
		return tr("ui.cancel", "Cancel")
	end, function()
		self.charImportMode = "GETACCOUNTNAME"
		self.charImportStatus = "Idle"
	end)
	self.controls.sessionPrivacySettings = new("ButtonControl", {"LEFT",self.controls.sessionCancel,"RIGHT"}, {8, 0, 120, 20}, function()
		return tr("import.privacy_settings", "Privacy Settings")
	end, function()
	...
	self.controls.sessionGo = new("ButtonControl", {"LEFT",self.controls.sessionInput,"RIGHT"}, {8, 0, 60, 20}, function()
		return tr("import.go", "Go")
	end, function()
		self:DownloadCharacterList()
	end)
	...
	self.controls.charSelectHeader = new("LabelControl", {"TOPLEFT",self.controls.sectionCharImport,"TOPLEFT"}, {6, 40, 200, 16}, function()
		return "^7" .. tr("import.select_character_header", "Choose character to import data from:")
	end)
	self.controls.charSelectHeader.shown = function()
		return self.charImportMode == "SELECTCHAR" or self.charImportMode == "IMPORTING"
	end
	self.controls.charSelectLeagueLabel = new("LabelControl", {"TOPLEFT",self.controls.charSelectHeader,"BOTTOMLEFT"}, {0, 6, 0, 14}, function()
		return "^7" .. tr("import.league", "League:")
	end)
	...
	self.controls.charImportHeader = new("LabelControl", {"TOPLEFT",self.controls.charSelect,"BOTTOMLEFT"}, {0, 16, 200, 16}, function()
		return "^7" .. tr("import.import_label", "Import:")
	end)
	self.controls.charImportTree = new("ButtonControl", {"LEFT",self.controls.charImportHeader, "RIGHT"}, {8, 0, 170, 20}, function()
		return tr("import.passive_tree_and_jewels", "Passive Tree and Jewels")
	end, function()
		if self.build.spec:CountAllocNodes() > 0 then
			main:OpenConfirmPopup(tr("import.character_import_title", "Character Import"), tr("import.overwrite_tree_warning", "Importing the passive tree will overwrite your current tree."), tr("import.import_button", "Import"), function()
	...
	self.controls.charImportTreeClearJewels = new("CheckBoxControl", {"LEFT",self.controls.charImportTree,"RIGHT"}, {90, 0, 18}, tr("import.delete_jewels", "Delete jewels:"), nil, tr("import.delete_jewels_tooltip", "Delete all existing jewels when importing."), true)
	self.controls.charImportItems = new("ButtonControl", {"LEFT",self.controls.charImportTree, "LEFT"}, {0, 36, 110, 20}, function()
		return tr("import.items_and_skills", "Items and Skills")
	end, function()
		self:DownloadItems()
		self:SetPredefinedBuildName()
	end)
	...
	self.controls.charImportItemsClearSkills = new("CheckBoxControl", {"LEFT",self.controls.charImportItems,"RIGHT"}, {85, 0, 18}, tr("import.delete_skills", "Delete skills:"), nil, tr("import.delete_skills_tooltip", "Delete all existing skills when importing."), true)
	self.controls.charImportItemsClearItems = new("CheckBoxControl", {"LEFT",self.controls.charImportItems,"RIGHT"}, {220, 0, 18}, tr("import.delete_equipment", "Delete equipment:"), nil, tr("import.delete_equipment_tooltip", "Delete all equipped items when importing."), true)
	self.controls.charImportItemsIgnoreWeaponSwap = new("CheckBoxControl", {"LEFT",self.controls.charImportItems,"RIGHT"}, {380, 0, 18}, tr("import.ignore_weapon_swap", "Ignore weapon swap:"), nil, tr("import.ignore_weapon_swap_tooltip", "Ignore items and skills in weapon swap."), false)
	self.controls.charBanditNote = new("LabelControl", {"TOPLEFT",self.controls.charImportHeader,"BOTTOMLEFT"}, {0, 50, 200, 14}, function()
		return "^7" .. tr("import.bandit_note", "Tip: After you finish importing a character, make sure you update the bandit choice,\nas it cannot be imported.")
	end)

	self.controls.charClose = new("ButtonControl", {"TOPLEFT",self.controls.charImportHeader,"BOTTOMLEFT"}, {0, 90, 60, 20}, function()
		return tr("import.close", "Close")
	end, function()
		self.charImportMode = "GETACCOUNTNAME"
		self.charImportStatus = "Idle"
	end)
	...
	-- Build import/export
	self.controls.sectionBuild = new("SectionControl", {"TOPLEFT",self.controls.sectionCharImport,"BOTTOMLEFT"}, {0, 18, 650, 182}, function()
		return tr("import.section.build_sharing", "Build Sharing")
	end)
	self.controls.generateCodeLabel = new("LabelControl", {"TOPLEFT",self.controls.sectionBuild,"TOPLEFT"}, {6, 14, 0, 16}, function()
		return "^7" .. tr("import.generate_code_label", "Generate a code to share this build with other Path of Building users:")
	end)
	self.controls.generateCode = new("ButtonControl", {"LEFT",self.controls.generateCodeLabel,"RIGHT"}, {4, 0, 80, 20}, function()
		return tr("import.generate", "Generate")
	end, function()
		self.controls.generateCodeOut:SetText(common.base64.encode(Deflate(self.build:SaveDB("code"))):gsub("+","-"):gsub("/","_"))
	end)
	self.controls.enablePartyExportBuffs = new("CheckBoxControl", {"LEFT",self.controls.generateCode,"RIGHT"}, {100, 0, 18}, tr("import.export_support", "Export Support"), function(state)
		self.build.partyTab.enableExportBuffs = state
		self.build.buildFlag = true 
	end, tr("import.export_support_tooltip", "This is for party play, to export support character, it enables the exporting of auras, curses and modifiers to the enemy"), false)
	self.controls.generateCodeOut = new("EditControl", {"TOPLEFT",self.controls.generateCodeLabel,"BOTTOMLEFT"}, {0, 8, 250, 20}, "", tr("import.code", "Code"), "%Z")
	self.controls.generateCodeOut.enabled = function()
		return #self.controls.generateCodeOut.buf > 0
	end
	self.controls.generateCodeCopy = new("ButtonControl", {"LEFT",self.controls.generateCodeOut,"RIGHT"}, {8, 0, 60, 20}, function()
		return tr("import.copy", "Copy")
	end, function()
	...
	self.creatingShareLink = false
	self.controls.generateCodeByLink = new("ButtonControl", { "LEFT", self.controls.exportFrom, "RIGHT"}, {8, 0, 100, 20}, function()
		return self.creatingShareLink and tr("import.creating_link", "Creating link...") or tr("import.share", "Share")
	end, function()
	...
	self.controls.generateCodeNote = new("LabelControl", {"TOPLEFT",self.controls.generateCodeOut,"BOTTOMLEFT"}, {0, 4, 0, 14}, function()
		return "^7" .. tr("import.share_note", "Note: this code can be very long; you can use 'Share' to shrink it.")
	end)
	self.controls.importCodeHeader = new("LabelControl", {"TOPLEFT",self.controls.generateCodeNote,"BOTTOMLEFT"}, {0, 26, 0, 16}, function()
		return "^7" .. tr("import.import_build_header", "To import a build, enter URL or code here:")
	end)
	...
	local importCodeHandle = function (buf)
		...
		self.importCodeDetail = colorCodes.NEGATIVE .. tr("import.invalid_input", "Invalid input")
		...
		for j=1,#buildSites.websiteList do
			if urlText:match(buildSites.websiteList[j].matchURL) then
				...
				self.importCodeDetail = colorCodes.POSITIVE .. tr("import.url_valid", "URL is valid (%s)", buildSites.websiteList[j].label)
		...
		self.importCodeValid = true
		self.importCodeDetail = colorCodes.POSITIVE .. tr("import.code_valid", "Code is valid")
	...
	local importSelectedBuild = function()
		...
		if self.controls.importCodeMode.selIndex == 1 then
			main:OpenConfirmPopup(tr("import.build_import_title", "Build Import"), colorCodes.WARNING .. tr("import.build_import_warning", "Warning:^7 Importing to the current build will erase ALL existing data for this build."), tr("import.import_button", "Import"), function()
				self.build:Shutdown()
		...
		elseif self.controls.importCodeMode.selIndex == 3 then
			...
			if self.build.compareTab then
				if self.build.compareTab:ImportBuild(self.importCodeXML, "Imported comparison") then
					...
					main:OpenMessagePopup(tr("import.import_error_title", "Import Error"), tr("import.import_comparison_failed", "Failed to import build for comparison."))
				end
			end
		else
			self.build:Shutdown()
	...
	self.controls.importCodeMode = new("DropDownControl", {"TOPLEFT",self.controls.importCodeIn,"BOTTOMLEFT"}, {0, 4, 200, 20}, self:GetImportCodeModeList())
	self.controls.importCodeMode.enabled = function()
		return (self.build.dbFileName or self.controls.importCodeMode.selIndex == 3) and self.importCodeValid
	end
	self.controls.importCodeGo = new("ButtonControl", {"LEFT",self.controls.importCodeMode,"RIGHT"}, {8, 0, 160, 20}, function()
		return self.importCodeFetching and tr("import.retrieving_paste", "Retrieving paste..") or tr("import.import_button", "Import")
	end, function()
...
function ImportTabClass:GetImportCodeModeList()
	return {
		tr("import.mode.current", "Import to this build"),
		tr("import.mode.new", "Import to a new build"),
		tr("import.mode.compare", "Import as comparison"),
	}
end

function ImportTabClass:RefreshLocale()
	if not self.controls.importCodeMode then
		return
	end
	local function refreshCheckBoxLabel(control, label)
		control.label = label
		control.labelWidth = DrawStringWidth(control.width - 4, "VAR", label or "") + 5
	end
	local selectedIndex = self.controls.importCodeMode.selIndex or 1
	self.controls.importCodeMode:SetList(self:GetImportCodeModeList())
	self.controls.importCodeMode.selIndex = math.min(selectedIndex, #self.controls.importCodeMode.list)
	refreshCheckBoxLabel(self.controls.charImportTreeClearJewels, tr("import.delete_jewels", "Delete jewels:"))
	refreshCheckBoxLabel(self.controls.charImportItemsClearSkills, tr("import.delete_skills", "Delete skills:"))
	refreshCheckBoxLabel(self.controls.charImportItemsClearItems, tr("import.delete_equipment", "Delete equipment:"))
	refreshCheckBoxLabel(self.controls.charImportItemsIgnoreWeaponSwap, tr("import.ignore_weapon_swap", "Ignore weapon swap:"))
	refreshCheckBoxLabel(self.controls.enablePartyExportBuffs, tr("import.export_support", "Export Support"))
	self.lastLocale = localize:GetLocale()
end
...
function ImportTabClass:Draw(viewPort, inputEvents)
	if self.lastLocale ~= localize:GetLocale() then
		self:RefreshLocale()
	end
	self.x = viewPort.x
	self.y = viewPort.y
