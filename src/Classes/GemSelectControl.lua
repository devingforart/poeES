...
local function getDisplayedGemName(gemData)
	return gemData and (gemData.displayName or gemData.name) or nil
end

local function getDisplayedTagString(gemData)
	return gemData and (gemData.displayTagString or gemData.tagString) or nil
end

local function getDisplayedGrantedEffectName(grantedEffect)
	return grantedEffect and (grantedEffect.displayName or grantedEffect.name) or nil
end

local function getDisplayedGrantedEffectDescription(grantedEffect)
	return grantedEffect and (grantedEffect.displayDescription or grantedEffect.description) or nil
end
	...
	if #self.searchStr > 0 then
		...
		for i, pattern in ipairs(patternList) do
			...
			for gemId, gemData in pairs(self.gems) do
				local searchName = (" "..(getDisplayedGemName(gemData) or gemData.name):lower())
				if self:FilterSupport(gemId, gemData) and not added[gemId] and (searchName:match(pattern) or (" "..gemData.name:lower()):match(pattern)) then
					addThisGem = true
					if #tagsList > 0 then
...
function GemSelectClass:Draw(viewPort, noTooltip)
	...
	if self.dropped then
		...
		for index = minIndex, maxIndex do
			...
			local gemText = gemData and getDisplayedGemName(gemData) or "<No matches>"
		...
		if self.hoverSel then
			...
			if calcFunc then
				...
				local gemInstance = {
						...
						enableGlobal2 = true,
						gemId = gemData.id,
						nameSpec = getDisplayedGemName(gemData),
						skillId = gemData.grantedEffectId,
						displayEffect = nil,
						gemData = gemData
					}
...
function GemSelectClass:AddGemTooltip(gemInstance)
	...
	if secondary and (not secondary.support or gemInstance.gemData.secondaryEffectName or gemInstance.gemData.displaySecondaryEffectName) then
		...
		self.tooltip:AddLine(fontSizeTitle, colorCodes.GEM..getDisplayedGrantedEffectName(grantedEffect), "FONTIN SC")
		self.tooltip:AddSeparator(10)
		self.tooltip:AddLine(fontSizeBig, "^x7F7F7F" .. getDisplayedTagString(gemInstance.gemData), "FONTIN SC")
		self:AddCommonGemInfo(gemInstance, grantedEffect, true)
		self.tooltip:AddSeparator(10)
		self.tooltip:AddLine(fontSizeTitle, colorCodes.GEM .. (gemInstance.gemData.displaySecondaryEffectName or gemInstance.gemData.secondaryEffectName or getDisplayedGrantedEffectName(grantedEffectSecondary)), "FONTIN SC")
		self.tooltip:AddSeparator(10)
...
function GemSelectClass:AddCommonGemInfo(gemInstance, grantedEffect, addReq, mergeStatsFrom)
	...
	local description = getDisplayedGrantedEffectDescription(grantedEffect)
	if description then
		local wrap = main:WrapString(description, 16, m_max(DrawStringWidth(16, "VAR", getDisplayedTagString(gemInstance.gemData)), 400))
