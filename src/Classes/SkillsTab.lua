...
local function getDisplayedGemName(gemData)
	return gemData and (gemData.displayName or gemData.name or gemData.nameSpec) or nil
end
	...
	self.controls.imbuedSupport = new("GemSelectControl", { "LEFT", self.controls.imbuedSupportLabel, "RIGHT" }, { 8, 0, 250, 20 }, self, 1, function(gemData, _, _, gemMatch, slotName)
		...
		if gemData and (type(gemData) == "string" or gemData.id) then
			...
			if updateDisplayGroup then
				...
				self.controls.imbuedSupport:SetText(getDisplayedGemName(gem))
			end
...
which comes from the following sources:]]
			...
			label = [[^7This is a special group created for the ']] .. activeGem.color .. (activeGem.grantedEffect and getDisplayedGrantedEffectName(activeGem.grantedEffect) or activeGem.nameSpec) .. [[^7' skill,
	...
	for _, child in ipairs(node) do
		...
		elseif child.attrib.skillId then
			...
			if grantedEffect then
				...
				gemInstance.nameSpec = getDisplayedGrantedEffectName(grantedEffect)
			end
		end
	...
	slot.quality.tooltipFunc = function(tooltip)
		if tooltip:CheckForUpdate(self.build.outputRevision, self.displayGroup) then
			...
			local addQualityLines = function(qualityList, grantedEffect)
				tooltip:AddLine(18, colorCodes.GEM..getDisplayedGrantedEffectName(grantedEffect))
...
function SkillsTabClass:FindSkillGem(nameSpec)
	...
	for i, pattern in ipairs(patternList) do
		...
		for gemId, gemData in pairs(self.build.data.gems) do
			local englishName = " "..gemData.name
			local displayName = " "..(getDisplayedGemName(gemData) or gemData.name)
			if englishName:match(pattern) or displayName:match(pattern) then
				if foundGemData then
					return "Ambiguous gem name '" .. nameSpec .. "': matches '" .. getDisplayedGemName(foundGemData) .. "', '" .. getDisplayedGemName(gemData) .. "'"
				end
	...
	for _, gemInstance in ipairs(socketGroup.gemList) do
		...
		if gemInstance.gemId then
			...
			if gemInstance.gemData then
				gemInstance.nameSpec = getDisplayedGemName(gemInstance.gemData)
...
function SkillsTabClass:SetDisplayGroup(socketGroup)
	...
	if socketGroup then
		...
		if socketGroup.imbuedSupport then
			...
			self.controls.imbuedSupport:SetText(getDisplayedGemName(gemId) or socketGroup.imbuedSupport)
	...
	for index, activeSkill in ipairs(socketGroup.displaySkillList) do
		...
		for _, skillEffect in ipairs(activeSkill.effectList) do
			tooltip:AddLine(20, string.format("%s%s ^7%d%s/%d%s",
				...
				getDisplayedGrantedEffectName(skillEffect.grantedEffect),
		...
		if activeSkill.minion then
			...
			tooltip:AddLine(20, string.format("%s%s ^7%d%s/%d%s",
				...
				getDisplayedGrantedEffectName(activeEffect.grantedEffect),
	...
	for _, gemInstance in ipairs(socketGroup.displayGemList or socketGroup.gemList) do
		if not gemShown[gemInstance] then
			...
			tooltip:AddLine(20, string.format("%s%s ^7%d%s/%d%s %s",
				gemInstance.color,
				(gemInstance.grantedEffect and getDisplayedGrantedEffectName(gemInstance.grantedEffect)) or getDisplayedGemName(gemInstance.gemData) or gemInstance.nameSpec,
