...
local gameLocalize = LoadModule("Modules/GameLocalization")
...
local function applyLocalizedSkillData(skill, localizedSkill)
	if not localizedSkill then
		return
	end
	if localizedSkill.name and localizedSkill.name ~= "" then
		skill.displayName = sanitiseText(localizedSkill.name)
	end
	if localizedSkill.baseTypeName and localizedSkill.baseTypeName ~= "" then
		skill.displayBaseTypeName = sanitiseText(localizedSkill.baseTypeName)
	end
	if localizedSkill.description and localizedSkill.description ~= "" then
		skill.displayDescription = sanitiseText(localizedSkill.description)
	end
end

local function applyLocalizedGemData(gem, localizedGem)
	if not localizedGem then
		return
	end
	if localizedGem.name and localizedGem.name ~= "" then
		gem.displayName = sanitiseText(localizedGem.name)
	end
	if localizedGem.baseTypeName and localizedGem.baseTypeName ~= "" then
		gem.displayBaseTypeName = sanitiseText(localizedGem.baseTypeName)
	end
	if localizedGem.secondaryEffectName and localizedGem.secondaryEffectName ~= "" then
		gem.displaySecondaryEffectName = sanitiseText(localizedGem.secondaryEffectName)
	end
	if localizedGem.tagString and localizedGem.tagString ~= "" then
		gem.displayTagString = sanitiseText(localizedGem.tagString)
	end
end
...
local localizedSkills = gameLocalize:GetOverlay("Skills")
for skillId, grantedEffect in pairs(data.skills) do
	grantedEffect.name = sanitiseText(grantedEffect.name)
	applyLocalizedSkillData(grantedEffect, localizedSkills and localizedSkills[skillId])
...
local localizedGems = gameLocalize:GetOverlay("Gems")
data.gemForSkill = { }
data.gemForBaseName = { }
data.gemsByGameId = { }
...
for gemId, gem in pairs(data.gems) do
    ...
    applyLocalizedGemData(gem, localizedGems and localizedGems[gem.variantId])
    setupGem(gem, gemId)
	if gem.vaalGem then
    ...
    for _, alt in ipairs{"AltX", "AltY"} do
        if gem.vaalGem and data.skills[gem.secondaryGrantedEffectId..alt] then
			...
			local displayBaseTypeName = data.skills[gem.secondaryGrantedEffectId..alt].displayBaseTypeName or data.skills[gem.secondaryGrantedEffectId..alt].baseTypeName or newGem.name
			newGem.displayName = "Vaal " .. displayBaseTypeName
			newGem.displayBaseTypeName = newGem.displayName
			newGem.displayTagString = gem.displayTagString
