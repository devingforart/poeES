...
function buildMode:Init(dbFileName, buildName, buildXML, convertBuild, importLink)
	...
	self.controls.modeImport.label = function()
		return tr("build.mode.import_export", "Import/Export Build")
	end
	...
	self.controls.modeNotes.label = function()
		return tr("build.mode.notes", "Notes")
	end
	...
	self.controls.modeConfig.label = function()
		return tr("build.mode.configuration", "Configuration")
	end
	...
	self.controls.modeTree.label = function()
		return tr("build.mode.tree", "Tree")
	end
	...
	self.controls.modeSkills.label = function()
		return tr("build.mode.skills", "Skills")
	end
	...
	self.controls.modeItems.label = function()
		return tr("build.mode.items", "Items")
	end
	...
	self.controls.modeCalcs.label = function()
		return tr("build.mode.calcs", "Calcs")
	end
	...
	self.controls.modeParty.label = function()
		return tr("build.mode.party", "Party")
	end
	...
	self.controls.modeCompare.label = function()
		return tr("build.mode.compare", "Compare")
	end
  ...
  controls.mainSocketGroup:CheckDroppedWidth(true)
	...
	if #controls.mainSocketGroup.list == 0 then
		...
		for i, activeSkill in ipairs(displaySkillList) do
			...
			t_insert(controls.mainSkill.list, { val = i, label = colourCoded or (activeSkill.activeEffect.grantedEffect.displayName or activeSkill.activeEffect.grantedEffect.name) })
		end
				...
				if not activeSkill.skillFlags.disable and (activeEffect.grantedEffect.minionList or activeSkill.minionList[1]) then
					...
					if activeSkill.minion then
						for _, minionSkill in ipairs(activeSkill.minion.activeSkillList) do
							t_insert(controls.mainSkillMinionSkill.list, minionSkill.activeEffect.grantedEffect.displayName or minionSkill.activeEffect.grantedEffect.name)
						end
