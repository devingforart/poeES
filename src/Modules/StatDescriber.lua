...
local function getRequestedLocale()
...
local function resolveDescriptorLanguage(descriptor)
	...
	local locale = normaliseLocaleName(getRequestedLocale())
...
return function(stats, scopeName, quality)
		...
		if (type(v) == "number" and v ~= 0) or (type(v) == "table" and (v.min ~= 0 or v.max ~= 0)) then	
			for depth, scope in ipairs(rootScope.scopeList) do
				if scope[s] then
					...
					if resolveDescriptorLanguage(descriptor) then
	...
	for _, descriptor in ipairs(descOrdered) do
		...
		local desc = matchLimit(resolveDescriptorLanguage(descriptor.description), val, quality)
		if desc then
