...
function main:Init()
	...
	self.gameAccounts = { }
	self.uiLanguage = "en"
	localize:SetLocale(self.uiLanguage)
	...
	self.controls.options = new("ButtonControl", {"BOTTOMLEFT",self.anchorMain,"BOTTOMLEFT"}, {0, 0, 68, 20}, function()
		return tr("main.options_button", "Options")
	end, function()
		self:OpenOptionsPopup()
	end)
	self.controls.about = new("ButtonControl", {"BOTTOMLEFT",self.anchorMain,"BOTTOMLEFT"}, {72, 0, 68, 20}, function()
		return tr("main.about_button", "About")
	end, function()
		self:OpenAboutPopup()
	end)
	self.controls.applyUpdate = new("ButtonControl", {"BOTTOMLEFT",self.anchorMain,"BOTTOMLEFT"}, {0, -24, 140, 20}, function()
		return "^x50E050" .. tr("main.update_ready", "Update Ready")
	end, function()
	...
	self.controls.checkUpdate.label = function()
		return launch.updateCheckRunning and launch.updateProgress or tr("main.check_for_update", "Check for Update")
	end
	...
	self.controls.forkLabel.label = function()
		return "^8" .. tr("main.community_fork", "PoB Community Fork")
	end
	...
	self.controls.versionLabel.label = function()
		local prefix = launch.versionBranch == "beta"
			and tr("main.beta_label", "Beta: %s", launch.versionNumber)
			or tr("main.version_label", "Version: %s", launch.versionNumber)
		return "^8" .. prefix .. (launch.versionBranch == "dev" and " (Dev)" or "")
	end
	...
	for _, node in ipairs(setXML[1]) do
		if type(node) == "table" then
			...
			elseif node.elem == "Misc" then
				...
				if node.attrib.uiLanguage then
					self.uiLanguage = node.attrib.uiLanguage
					localize:SetLocale(self.uiLanguage)
				end
...
function main:OpenOptionsPopup(savedState)
	...
	drawSectionHeader("app", tr("main.application_options", "Application options"))

	controls.uiLanguage = new("DropDownControl", { "TOPLEFT", controls.sectionAnchor, "TOPLEFT" }, { currentX + defaultLabelPlacementX, currentY, 150, 18 }, localize:GetLocaleList(), function(index, value)
		self.uiLanguage = value.id
		localize:SetLocale(value.id)
		self:ClosePopup()
		self:OpenOptionsPopup(savedState)
	end)
	controls.uiLanguageLabel = new("LabelControl", { "RIGHT", controls.uiLanguage, "LEFT" }, { defaultLabelSpacingPx, 0, 0, 16 }, function()
		return "^7" .. tr("main.language", "Language:")
	end)
	controls.uiLanguage:SelByValue(self.uiLanguage, "id")
	...
	controls.connectionProtocolLabel = new("LabelControl", { "RIGHT", controls.connectionProtocol, "LEFT" }, { defaultLabelSpacingPx, 0, 0, 16 }, function()
		return "^7" .. tr("main.connection_protocol", "Connection Protocol:")
	end)
	controls.connectionProtocol.tooltipText = tr("main.connection_protocol_tooltip", "Changes which protocol is used when downloading updates and importing builds.")
	...
	controls.proxyLabel = new("LabelControl", { "RIGHT", controls.proxyType, "LEFT" }, { defaultLabelSpacingPx, 0, 0, 16 }, function()
		return "^7" .. tr("main.proxy_server", "Proxy server:")
	end)
	...
	controls.dpiScaleOverride = new("DropDownControl", { "TOPLEFT", controls.sectionAnchor, "TOPLEFT" }, { currentX + defaultLabelPlacementX, currentY, 150, 18 }, {
		{ label = tr("main.use_system_default", "Use system default"), percent = 0 },
	...
	controls.dpiScaleOverrideLabel = new("LabelControl", { "RIGHT", controls.dpiScaleOverride, "LEFT" }, { defaultLabelSpacingPx, 0, 0, 16 }, function()
		return "^7" .. tr("main.ui_scaling_override", "UI scaling override:")
	end)
	controls.dpiScaleOverride.tooltipText = tr("main.ui_scaling_tooltip", "Overrides Windows DPI scaling inside Path of Building.\nChoose a percentage between 100% and 250% or revert to the system default.")
	...
	controls.buildPathLabel = new("LabelControl", { "RIGHT", controls.buildPath, "LEFT" }, { defaultLabelSpacingPx, 0, 0, 16 }, function()
		return "^7" .. tr("main.build_save_path", "Build save path:")
	end)
	...
	controls.cancel = new("ButtonControl", { "BOTTOM", nil, "BOTTOM" }, {45, -10, 80, 20}, "Cancel", function()
		...
		self.uiLanguage = savedState.uiLanguage
		localize:SetLocale(self.uiLanguage)
		SetDPIScaleOverridePercent(self.dpiScaleOverridePercent)
		main:ClosePopup()
	end)
	...
	local popup = self:OpenPopup(popupWidth, popupHeight, tr("main.options_title", "Options"), controls, "save", nil, "cancel", useScrollBar and scrollBarFunc or nil)
...
function main:OpenConfirmPopup(title, msg, confirmLabel, onConfirm, extraLabel, onExtra)
	...
	if extraLabel and onExtra then
		...
		placeButton(cancelWidth, tr("ui.cancel", "Cancel"), function() end)
		...
		t_insert(controls, new("ButtonControl", nil, {5 + m_ceil(confirmWidth/2), 40 + numMsgLines * 16, confirmWidth, 20}, tr("ui.cancel", "Cancel"), function()
			main:ClosePopup()
		end))
