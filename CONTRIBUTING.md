...
### Exporting official Spanish game text

If you want to generate the official Spanish overlays used by the localisation runtime:

1. Set `POB_EXPORT_LOCALE=es` before launching the exporter.
2. Launch the exporter normally.
3. Run `statdesc`.
4. Run `skills`.
...
If you only want to refresh the locale overlays without rewriting the main generated `Skills` and `Gems` data, also set:

```text
POB_ONLY_LOCALE_EXPORT=1
```

See `./docs/official-es-localization.md` for the runtime behaviour and limitations.
